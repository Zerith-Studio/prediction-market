...
<Warning>
A real bug this discipline caught: JavaScript's `Number(bigint)` silently rounds values
≥ 2⁵³. A 63-bit random `salt` signed one value and transmitted another, so ~99.9% of
orders failed signature verification. `randomSalt()` now draws below 2⁵³ so the
bigint → Number → JSON → uint64 round-trip is exact (fixed in `mobile/`; the same flaw
is flagged in `frontend/lib/borsh.ts` per `progress.md`).
</Warning>
...
Measured in the crank's tests: legacy 1421&nbsp;B ❌ vs v0 1116&nbsp;B ✅.
