---
title: "Architecture"
description: "How the on-chain settlement program and the backend matching service fit together across a frozen interface contract."
---

PitchMarket has two halves — an on-chain Anchor settlement program and an off-chain Go
backend — joined by a frozen boundary document,
[`docs/interface-contract.md`](https://github.com/Zerith-Studio/prediction-market/blob/main/docs/interface-contract.md),
which pins the account layouts, instruction arguments, transaction layouts, and REST/WS
surface both sides build against. This page maps how they fit together.
...
## The on-chain settlement program (`programs/pitchmarket/`)
...
Deployed on Solana devnet at `3fdgRPcZnwWcaGi197dkZDyq24VHoWJcGzKTVfMxNPWs` — see
...
- **`mobile/`** — Expo (React Native) app covering the core loop: markets, trade,
  deposit, portfolio. Shares the signed-order and live-market client layer with the web
  frontend.
